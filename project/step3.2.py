...
####Function to carve the voxels, calculating A
###All the functions are used for the main function voxel_carving
...
def preprocess(model):
    ...
    vertices -= center
    model.vertices = open3d.utility.Vector3dVector(vertices / scale)
    return model
...
def voxel_carving(mesh,
    ...
    camera_sphere = open3d.io.read_triangle_mesh(camera_path)

    # setup dense voxel grid
    voxel_carving = open3d.geometry.VoxelGrid.create_dense(
        width=cubic_size,
        height=cubic_size,
        depth=cubic_size,
    ...
    # carve voxel grid
    pcd_agg = open3d.geometry.PointCloud()
    ...
    for cid, xyz in enumerate(camera_sphere.vertices):
        ...
        pcd_agg += open3d.geometry.PointCloud.create_from_depth_image(
        ...
        if use_depth:
            voxel_carving.carve_depth_map(open3d.geometry.Image(depth), param)
        else:
            voxel_carving.carve_silhouette(open3d.geometry.Image(depth), param)
    ...
    if surface_method == 'pointcloud':
        voxel_surface = open3d.geometry.VoxelGrid.create_from_point_cloud_within_bounds(
            pcd_agg,
            voxel_size=cubic_size / voxel_resolution,
            min_bound=(-cubic_size / 2, -cubic_size / 2, -cubic_size / 2),
            max_bound=(cubic_size / 2, cubic_size / 2, cubic_size / 2))
    elif surface_method == 'mesh':
        voxel_surface = open3d.geometry.VoxelGrid.create_from_triangle_mesh_within_bounds(
            mesh,
            voxel_size=cubic_size / voxel_resolution,
            min_bound=(-cubic_size / 2, -cubic_size / 2, -cubic_size / 2),
            max_bound=(cubic_size / 2, cubic_size / 2, cubic_size / 2))
...
output_filename = os.path.abspath("data/LabeledDB_new/Human/voxalized.off")
