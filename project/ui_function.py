...
def feature_extraction(mesh, mesh_path):
    ...
    surface_area = calc_mesh_area(mesh)
    ...
    mesh_df = mesh_df.append(
        pd.DataFrame([[mesh_id, class_shape, surface_area, sphericity, bounding_box_volume, diameter, eccentricity,
                       a3, d1, d2, d3, d4]],
                     columns=mesh_df.columns))
    return mesh_df
...
def matching(single_feature, has_weight):
    ...
    for _, row in database_feature.iloc[:, 2:len(database_feature.columns)].iterrows():
        ...
        emd = euclidean_distance(single_feature[5:len(single_feature)], distribution_vector)
