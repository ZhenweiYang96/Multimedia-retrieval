...
def mesh_to_data(directory, name):
    ...
    mesh_df = pd.DataFrame(
        columns=['id', 'class_shape', 'num_faces', 'num_vertices', 'type_shape', 'bounding_box_points', 'extent_length',
    ...
    for file in file_list:
        ...
        for off_file in os.listdir(file_path):
            ...
            if meshes_file.endswith(".off"):
                ...
                extent_length = np.asarray(open3d.geometry.AxisAlignedBoundingBox.get_extent(bounding_box))  # get length
                extent_x = extent_length[0]
                extent_y = extent_length[1]
                extent_z = extent_length[2]
                if not ('78' in meshes_file or '281' in meshes_file):
                    mesh_df = mesh_df.append(pd.DataFrame([[mesh_id, class_shape, mesh_triangles, mesh_vertices, faces_type,
                                                        bounding_box_points, extent_length, extent_x, extent_y, extent_z]],
